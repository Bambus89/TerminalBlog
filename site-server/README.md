...
- Einzelnes Binary, keine Abhängigkeiten
...
```bash
# Option A: Server als root starten (nicht empfohlen)
# Option B: Den Service-User zur Gruppe ssl-cert hinzufügen (Debian/Ubuntu)
sudo usermod -aG ssl-cert dein-user

# Option C: Leserechte für die Live-Verzeichnisse setzen
...
Wenn der site-server auf Port 443 (HTTPS) läuft und du zusätzlich Port 80 auf HTTPS umleiten möchtest, kannst du eine zweite Instanz starten:

```bash
# Zusätzliche config für HTTP-Redirect (config-redirect.json)
{
  "webroot": "/var/www/html",
  "port": 80
}
...
Oder einfacher: Eine Firewall-Regel (iptables/nftables) die Port 80 auf 443 umleitet.
...
```json
{
  "webroot": "/home/christian/website",
  "port": 3000,
  "cert_file": "",
  "key_file": ""
}
...
Oder die Zertifikats-Felder einfach weglassen – sie sind optional:
...
4. **TLS-Zertifikat** – Pfad zum Fullchain (optional, leer = kein TLS)
5. **TLS-Schlüssel** – Pfad zum Private Key
6. **Service-Benutzer** – unter welchem User der Server läuft
7. **Service-Name** – Name des systemd-Service (Default: `site-server`)
...
make linux-arm64  # Linux arm64 (Raspberry Pi etc.)
